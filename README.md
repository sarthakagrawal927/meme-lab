...
Jev does **not** implement either local model route. The public Worker uses classifier.dev's Jev fast tier to rank a 30-item semantic shortlist and to guard serious help-seeking prompts. When both a narrator and another participant are present, three parallel Jev calls select distinct memes for **My reaction**, **Their side**, and **The situation**. Jev scores return directly as fit scores, and candidates below the visible-fit threshold are omitted. If any perspective call fails, the Worker retries the existing general Jev ranking before using the structured Workers AI selector as the final fallback.
