...
Jev does **not** implement either local model route. The public Worker uses classifier.dev's Jev fast tier to rank a 30-item semantic shortlist and to guard serious help-seeking prompts. It returns one primary result plus four backups. When both a narrator and another participant are present, three parallel Jev calls select distinct memes for **My reaction**, **Their side**, and **The situation**, then fill remaining slots with the strongest unused lens matches. Jev scores return directly as fit scores, including low scores on weaker backups. If any perspective call fails, the Worker retries the existing general Jev ranking before using the structured Workers AI selector as the final fallback.
