...
Jev does **not** implement either local model route. With the production secret configured, the public Worker calls TypeSafe directly: one structured request independently rates every candidate in the 30-item semantic shortlist. It returns one primary result plus four backups with ordinal fit labels instead of presenting Jev confidence as a probability that the meme is correct. When both a narrator and another participant are present, one batched request selects distinct memes for **My reaction**, **Their side**, and **The situation**, then one request rates the selected five on the same ordinal scale. If perspective selection fails, the Worker retries the general Jev ranking. If Jev is rate-limited, the Worker returns the first five semantic-retrieval matches marked low confidence and weak fit. It never invokes a large text-generation fallback.

Production should set `TYPESAFE_API_KEY` as a Worker secret. When present, the Worker uses the direct TypeSafe Jev API for safety, fit, and perspective decisions. The key is sent only as a bearer token to `api.typesafe.ai`; it is never included in browser responses or checked into the repository. Without the secret, local development continues to use classifier.dev.
