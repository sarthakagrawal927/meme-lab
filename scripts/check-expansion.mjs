...
import {parseJsonl,promoteStage1000Status,validateExpansionRecords,validateEvalCases,validateSourceCandidates,validateStageCoverageCases,validateStage1000Cases,validateCoverageMetadata,validateMeaningSpecificMetadata,validateStages,expansionStatus,withCandidatePool} from '../src/expansion.mjs';
...
const stage3000Source=parseJsonl(await readFile(resolve(root,'expansion/sources/stage-3000-source-phase1.jsonl'),'utf8'),'stage-3000 source phase 1');
...
validateSourceCandidates(stage3000Source,{knownNames:[...stage300Source,...stage1000Source].map(record=>record.name),minimum:800});
if(stage3000Source.length!==stage3000Acquisition.retained_candidates) throw new Error('Stage-3000 source acquisition count does not match its generated pool.');
...
const status={
  ...
  stage_3000_source:{status:'building',raw_source_records:stage3000Acquisition.retained_candidates,target_additions:stage3000Acquisition.target_gap,remaining_source_gap:stage3000Acquisition.remaining_source_gap,provider_total:stage3000Acquisition.provider_total,human_validated:false}
};
