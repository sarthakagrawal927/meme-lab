...
export class Store {
  ...
  async save(type, value) {
    if (!['run','feedback','experiment','experiment_review','candidate_review'].includes(type)) throw new Error('Invalid event type.');
  ...
  async all() {
    await mkdir(this.directory,{recursive:true,mode:0o700});
    const names=(await readdir(this.directory)).filter(n=>/^(run|feedback|experiment|experiment_review|candidate_review)-[a-f0-9-]{36}\.json$/.test(n));
